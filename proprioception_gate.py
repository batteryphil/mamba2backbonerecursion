...
class GeometricProprioceptionGate(nn.Module):
    ...
    Adapted for mamba-1.4b: d_model=2048.
    """

    def __init__(self, d_model: int = 2048, window_size: int = 8) -> None:
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        # Signal 2: Drift — directional instability (inverted cosine similarity)
        drift = (1.0 - F.cosine_similarity(h, h_prev, dim=-1)).unsqueeze(-1)

        # Signal 3: Stagnation — INVERTED variance (the key degeneration fix)
        ...
        # Combine all three signals and apply learned dampening
