...
sys.stdout = Tee('training.log')
print(f"  Live log: training.log  (watch with: tail -f training.log)")
...
SEQ_LEN     = 256
BATCH_SIZE  = 4            # Reduced for RTX 3060 12GB
ACCUM       = 4
STEPS       = 10_000       # Sufficient for convergence from pretrained base
LOG_EVERY   = 50
VAL_EVERY   = 500
EARLY_STOP_ACC   = 95.0
EARLY_STOP_COUNT = 3
RESUME_FROM = "mamba2_130m_v34_rope_best.pt"  # Resume from step 4000 best
...
def _parse_v2_chain(answer: str) -> list[str] | None:
    ...
    Each reasoning step becomes one loop target. The final answer
    and <HALT> are always the last two entries.
    ...
    # Parse reasoning steps (separated by '; ')
    steps = [s.strip() for s in reasoning.split(';') if s.strip()]

    for step in steps:
        # Arithmetic: "Compute: 7 + 2 = 9" → target '9'
        arith_m = re.search(r'=\s*(\S+)\s*$', step)
        if arith_m:
            targets.append(arith_m.group(1))
            continue
        # Assignment: "X ← Y = blue" → target 'blue'
        assign_m = re.search(r'←\s*(\S+?)(?:\s*=\s*(\S+))?\s*$', step)
        if assign_m:
            targets.append(assign_m.group(2) or assign_m.group(1))
            continue
        # Containment: "card → shelf" → target 'shelf'
        cont_m = re.search(r'→\s*(\S+)\s*$', step)
        if cont_m:
            targets.append(cont_m.group(1))
            continue
        # Given: "Given: 7 + 2" → skip (no target yet)
        if step.lower().startswith('given'):
            continue
        # Step N: ... → extract last word as target
    ...
    # Always end with final answer + HALT
...
def load_training_data() -> list[dict]:
    ...
    for fname, label in [("system2_logic_v3_curriculum.json", "v3_curriculum"),
        ...
        ok = 0
        for item in data:
            # v2_clean: answer contains [Reasoning]...[Answer]...<HALT>
            if "[Reasoning]" in item.get("answer", ""):
                ct = _parse_v2_chain(item["answer"])
                # Reconstruct full text: prompt + answer for tokenizer
        ...
        if ok > 0:
            break  # Use best available dataset, don't duplicate
...
if not samples:
    raise RuntimeError("No training data. Check system2_logic_v2_clean.json or v33 exists.")
