...
print("Loading model for Gate Ablation...")
...
# Locate RAM and ALU dims
base_gate = model.lifeline_gate.data.clone()
ram_idx = torch.where(base_gate > 1.01)[0]
alu_idx = torch.where(base_gate < 0.97)[0]
print(f"RAM dims (>1.01): {len(ram_idx)}")
print(f"ALU dims (<0.97): {len(alu_idx)}")

def evaluate_ablation(ram_scale: float, alu_scale: float, debug=False):
    new_gate = base_gate.clone()
    new_gate[ram_idx] = base_gate[ram_idx] * ram_scale
    new_gate[alu_idx] = base_gate[alu_idx] * alu_scale
    model.lifeline_gate.data.copy_(new_gate)
    
    test_chains = [
        ("P = telescope. Q = P. R = Q. What is R?\nAnswer:", ["Q", "P", "telescope", "<HALT>"]),
        ("AA = moon. BB = AA. CC = BB. What is CC?\nAnswer:", ["BB", "AA", "moon", "<HALT>"]),
        ("X = algorithm. Y = X. Z = Y. What is Z?\nAnswer:", ["Y", "X", "algorithm", "<HALT>"]),
        ("M = winter. N = M. O = N. What is O?\nAnswer:", ["N", "M", "winter", "<HALT>"]),
        ("A = ocean. B = A. C = B. What is C?\nAnswer:", ["B", "A", "ocean", "<HALT>"]),
    ]
    
    ptr_acc = 0.0; ptr_count = 0
    val_acc = 0.0; val_count = 0
    hlt_acc = 0.0; hlt_count = 0
    ptr_margin = 0.0; val_margin = 0.0

    for prompt, expected in test_chains:
        ...
        if debug: print(f"  Prompt: {prompt.strip()}")
        
        with torch.no_grad():
            x = model.backbone.embedding(ids_)
            residual = None
            for layer in model.all_layers: x, residual = layer(x, residual)
            x_prompt = x.clone().detach()
            
            for loop_i in range(len(expected)):
                ...
                marg = (topk.values[0] - topk.values[1]).item()
                top_tok = tokenizer.decode([topk.indices[0]]).strip()
                if topk.indices[0] == HALT_ID: top_tok = "<HALT>"
                
                exp_tok = expected[loop_i]
                is_hit = (top_tok.lower() == exp_tok.lower() or (top_tok=="<HALT>" and exp_tok=="<HALT>"))
                
                if debug:
                    print(f"    L{loop_i+1}: Exp={exp_tok:10} | Got={top_tok:10} | Hit={is_hit} | Marg={marg:.2f}")
                
                if loop_i < len(expected) - 2:
                    ptr_acc += float(is_hit)
                    ptr_margin += marg
                    ptr_count += 1
                elif loop_i == len(expected) - 2:
                    val_acc += float(is_hit)
                    val_margin += marg
                    val_count += 1
                elif loop_i == len(expected) - 1:
                    hlt_acc += float(is_hit)
                    hlt_count += 1

    return (ptr_acc/ptr_count*100, val_acc/val_count*100, hlt_acc/hlt_count*100, 
            ptr_margin/ptr_count, val_margin/val_count)

print("\n--- BASELINE DEBUG (1.0, 1.0) ---")
evaluate_ablation(1.0, 1.0, debug=True)

print("\n--- FULL ZERO ABLATION (Lifeline Removed) ---")
print("RAM Scale 0.0, ALU Scale 0.0 (and neutrality zeroed)")
# Temporary hack to test ZERO entirely
old_gate = model.lifeline_gate.data.clone()
model.lifeline_gate.data.fill_(0.0)
evaluate_ablation(0.0, 0.0, debug=True)
model.lifeline_gate.data.copy_(old_gate)

print("\n--- RAM ABLATION (Suppressing Value Retrieval) ---")
print("RAM Scale | Ptr Acc | Val Acc | Hlt Acc | Ptr Marg | Val Marg")
for rs in [1.0, 0.75, 0.5, 0.25, 0.0]:
    pa, va, ha, pm, vm = evaluate_ablation(rs, 1.0)
    print(f"     {rs:.2f} |  {pa:5.1f}% |  {va:5.1f}% |  {ha:5.1f}% |    {pm:5.1f} |    {vm:5.1f}")

print("\n--- ALU ABLATION (Amplifying Control Routing into failure) ---")
print("ALU Scale | Ptr Acc | Val Acc | Hlt Acc | Ptr Marg | Val Marg")
for as_ in [1.0, 1.05, 1.10, 1.15, 1.25]:
    pa, va, ha, pm, vm = evaluate_ablation(1.0, as_)
    print(f"     {as_:.2f} |  {pa:5.1f}% |  {va:5.1f}% |  {ha:5.1f}% |    {pm:5.1f} |    {vm:5.1f}")

print("\n--- CROSS-ABLATION (The Double Dissociation) ---")
print("RAM/ALU       | Ptr Acc | Val Acc | Hlt Acc | Ptr Marg | Val Marg")
pa, va, ha, pm, vm = evaluate_ablation(0.25, 1.15)
print(f"0.25/1.15 (A) |  {pa:5.1f}% |  {va:5.1f}% |  {ha:5.1f}% |    {pm:5.1f} |    {vm:5.1f}")
pa, va, ha, pm, vm = evaluate_ablation(1.15, 0.25)
print(f"1.15/0.25 (B) |  {pa:5.1f}% |  {va:5.1f}% |  {ha:5.1f}% |    {pm:5.1f} |    {vm:5.1f}")
