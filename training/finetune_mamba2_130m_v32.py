"""
finetune_mamba2_130m_v32.py — Mamba2 + Latent Forcing + <HALT> + Prompt Lifeline (v32)
========================================================================================
Addresses the two root causes of v31's failure on multi-hop chains:

PROBLEM 1: SSM Memory Decay (Associative Recall Bottleneck)
  v31 logs: X→Y→'1' (should be Apple), A→B→C→<HALT> (should be moon)
  Cause: Mamba's fixed-size state matrix is repeatedly mutated for pointer
  logic. By loop 3, the target word's token ID is "washed out" and the model
  falls back to pretrained priors ('1', 'blue', 'downhill').

FIX 1: Prompt Lifeline (x_prompt skip connection)
  Before the loop, save the uncorrupted prompt hidden state.
  At EVERY loop iteration, re-inject it as a residual:
    x = x + x_prompt
  This grounds the loop in the original text at each step, ensuring the
  target word's vector (e.g., "Apple") is always available even at loop 8.

PROBLEM 2: Premature HALT (Statistical Loop Counter)
  v31 trained on data where 80%+ of chains were 1-3 hops.
  Model learned: "Loop 3 → P(HALT) ≈ 1.0" as a shortcut.

FIX 2: Flat Hop Distribution + Random Vocabulary
  Training data: system2_logic_v32.json (built by v32_data_builder.py)
  - 6,000 samples each of 1/2/3/4/5-hop chains (flat distribution)
  - Chain values: random rare words from token ids 5k-45k
    (e.g., "carburetor", "photosynthesis", not "red"/"moon")
  - Forces model to learn a universal COPY circuit, not semantic buckets
"""
import torch
...
LOG_EVERY   = 50
VAL_EVERY   = 500
EARLY_STOP_ACC   = 95.0   # 95% on full vocab = real proof
EARLY_STOP_COUNT = 3
RESUME_FROM = ""
SAVE_PATH   = "mamba2_130m_v32_lifeline.pt"

LOOP_HEADDIM = 64
LOOP_D_STATE = 64

print(f"\n{'='*66}", flush=True)
print(f"  Mamba2-130m v32 — Latent Forcing + Prompt Lifeline", flush=True)
print(f"  FIX 1: x_prompt skip connection — bypasses SSM memory decay", flush=True)
print(f"  FIX 2: random vocab + flat hop dist — destroys loop-counter prior", flush=True)
print(f"  NO POINTER MASK — full 50k vocab softmax", flush=True)
...
print(f"  Vocab size: {len(tokenizer):,}  (NO mask)")
print(f"  <HALT> id: {HALT_ID}\n")


def _parse_chain_v32(sample: dict) -> list[str] | None:
    """Parse pre-built v32 sample — answer already known."""
    return [sample["answer"], "<HALT>"] if sample.get("n_hops", 0) == 1 else None


def _parse_chain(text: str) -> list[str] | None:
    """Extract per-loop targets from chain text, append <HALT>."""
...
def find_answer_start(ids: list[int]) -> int:
    ...
    Uses len(ids)-n+1 so Answer: at the very end (v32 format) is found.
    Returns len(ids) when Answer: is at the tail — the model predicts
    from the last real position in that case.
    """
    for boundary in (
        ...
        n = len(boundary)
        for i in range(len(ids) - n + 1):          # +1: catches tail position
            if ids[i:i + n] == boundary:
                pos = i + n
                return pos if pos <= len(ids) else len(ids) - 1
    return -1


# ── LoRA Linear ───────────────────────────────────────────────────────────────
class LoRALinear(nn.Module):
    """LoRA adapter — zero lora_B, near-identity warmup."""

    def __init__(self, linear: nn.Linear, rank: int = 8, alpha: float = 16.0):
        """Init from base linear, same dtype."""
        super().__init__()
...
# ── Recursive Mamba2-130m v32 ─────────────────────────────────────────────────
class RecursiveMamba2_v32(nn.Module):
    """
    Mamba2-130m v32 — Latent Forcing + Prompt Lifeline + <HALT>.

    The Prompt Lifeline:
        x_prompt = hidden state after initial base forward pass (before loops)
        At every loop: x = x + x_prompt
        This re-injects the uncorrupted prompt embedding containing the value
        token (e.g., 'Apple') at every iteration, bypassing SSM memory decay.
        The loop's job becomes: route to the right value in x_prompt, not
        reconstruct it from a degraded recurrent state.

    No pointer mask — full 50,279-token softmax always.
    """

    MAX_LOOPS: int = MAX_LOOPS

    def __init__(self, backbone: MambaLMHeadModel, lora_rank: int = 8):
        """Freeze bottom, LoRA top, Mamba2 loop engine, lifeline gate."""
        super().__init__()
        ...
        # Learnable gate scalar for the lifeline injection strength
        # Starts at 1.0 (full injection), model can learn to modulate
        ...
        n_new  = (sum(p.numel() for p in self.step_emb.parameters()) +
                  ...
                  sum(p.numel() for p in self.mamba2_core.parameters()) + 1)
        total  = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"  LoRA params:     {n_lora:,}")
        print(f"  Loop + lifeline: {n_new:,}")
        print(f"  Total trainable: {total:,}")
        print(f"  Pointer mask:    NONE (full {len(tokenizer):,}-token softmax)")
        print(f"  Prompt lifeline: x = x + gate * x_prompt at every loop\n")
    ...
    def forward(
        ...
        """
        Base forward pass → save x_prompt → loop with lifeline injection.
        Training: full-vocab Latent Forcing loss.
        Inference: halt on <HALT> prediction. No mask.
        """
        ...
        # ── THE PROMPT LIFELINE ─────────────────────────────────────────────
        # Save uncorrupted base hidden state BEFORE any loop modifications.
        # This contains the full, fresh encoding of "Apple", "moon", etc.
        ...
        if self.training and chain_targets is not None:
            ...
            for loop_i in range(n_loops):
                # LIFELINE: re-inject prompt at every loop
                ...
                for b in range(B):
                    ...
                    logits_b = logits_step[b, as_ - 1, :]  # full vocab
        ...
        # ── Inference — lifeline active, no mask, halt on <HALT> ──────────────
        else:
            trace:      list[tuple] = []
            last_answer = ""

            for loop_i in range(self.MAX_LOOPS):
                # LIFELINE re-injection at inference too
                x = x + self.lifeline_gate * x_prompt
                ...
                p   = torch.softmax(lg[0, -1, :], dim=-1)   # full vocab
                tid = p.argmax().item()
...
def load_training_data() -> list[dict]:
    """Load v32 random-vocab data (primary) + v1 chains for diversity."""
    samples: list[dict] = []
    ...
    def _prep(text: str, chain_targets: list[str]) -> dict | None:
        ...
        if ans_start < 1:
            return None
        # Clamp: when Answer: is at the very tail (v32 format), point to last token
    ...
    if os.path.exists("system2_logic_v32.json"):
        ...
        print(f"  v32 random-vocab: {added:,} | hop dist: {dict(sorted(hop_dist.items()))}")
    else:
        print(f"  ⚠️  system2_logic_v32.json not found — run v32_data_builder.py first")

    # Supplement: original system2 chains for lexical diversity
    if os.path.exists("system2_logic_v1.json"):
        ...
        for item in v1_data:
            ...
            if ct and len(ct) >= 3:
                s = _prep(text, ct)
                if s: samples.append(s); v1_added += 1
        print(f"  v1 supplement:    {v1_added:,}")
...
model = RecursiveMamba2_v32(base_model, lora_rank=LORA_RANK).to(DEVICE)

# Warm-start from v31 — similar architecture, transfers LoRA + loop engine
...
if not samples:
    raise RuntimeError("No training data found. Run v32_data_builder.py first.")
...
g1_params = (
    [model.step_emb.weight, model.lifeline_gate]
...
print(f"{'─'*66}")
print(f"  Mamba2-130m v32 | NO MASK | Prompt Lifeline | {STEPS} steps")
print(f"  Data: random vocab (5k-45k token range) + flat 1-5 hop dist")
...
for step in range(1, STEPS + 1):
    ...
    if step % LOG_EVERY == 0:
        ...
        t0   = time.time()
        gate = model.lifeline_gate.item()
        print(
            f"  Step {step:5d} | Loss: {al:.4f} | AllLoop: {aa:5.1f}%"
            f" | Answer: {fa:5.1f}% | Halt: {ha:5.1f}%"
            f" | Gate: {gate:.3f} | LR: {optimizer.param_groups[0]['lr']:.1e}"
    ...
    if step % VAL_EVERY == 0:
        ...
        print(f"  Val AllLoop: {vaa:5.1f}% | Answer: {vfa:5.1f}% | Halt: {vha:5.1f}%"
              f"  [full vocab, no mask]", flush=True)
        ...
        if vaa > best_val_acc:
            ...
            torch.save({"model_state_dict": model.state_dict(),
                        ...
                        "lifeline": True, "backbone": MODEL_ID}, best_path)
        ...
        if vaa >= EARLY_STOP_ACC:
            ...
            if early_stop_hits >= EARLY_STOP_COUNT:
                print(f"\n  ✅ EARLY STOP @ step {step}. Val={vaa:.1f}% — REAL PROOF", flush=True)
                torch.save({"model_state_dict": model.state_dict(),
                            "step": step, "halt_id": HALT_ID,
                            "no_mask": True, "lifeline": True}, SAVE_PATH)
                break
        else:
            early_stop_hits = 0
        print(f"  {'─'*52}\n", flush=True)
...
print("=" * 66)
print("  v32 Inference — Mamba2 + Lifeline, no mask, full vocab")
print("=" * 66)
with torch.no_grad():
    for prompt, expected in tests:
        ...
        print(f"  {match} {prompt.splitlines()[0][:60]!r}")
        print(f"     {chain_str}")
