...
class Mamba3ReasoningBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, complex_state: torch.Tensor = None) -> tuple:
        B, L, _ = x.shape
        x_in = self.in_proj(x)
        x_in = x_in.view(B, L, self.n_channels, self.d_model).unsqueeze(-1)
        if complex_state is None:
            complex_state = torch.zeros(B, L, self.n_channels, self.d_model, self.d_state, dtype=torch.cfloat, device=x.device)
        A = torch.complex(torch.cos(self.A_theta), torch.sin(self.A_theta)).unsqueeze(0).unsqueeze(0)
        B_mat = torch.complex(self.B_real, self.B_imag).unsqueeze(0).unsqueeze(0)
        C_mat = torch.complex(self.C_real, self.C_imag).unsqueeze(0).unsqueeze(0)
        x_c = x_in.to(torch.cfloat)
        new_state = A * complex_state + B_mat * x_c
        y_c = C_mat * new_state
        y_real = y_c.real.sum(dim=-1)
        y_flat = y_real.view(B, L, self.n_channels * self.d_model)
...
class RecursiveMamba130M(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, max_loops_override: int = None) -> tuple:
        ...
        for step_i in range(limit):
            ...
            x, complex_state = self.mamba3_core(x, complex_state)
...
if __name__ == "__main__":
    ckpts = ["mamba3_finetuned_v24_MaxN_3.pt", "mamba3_finetuned_v24_MaxN_2.pt"]
    for ckpt in ckpts:
        print(f"\n========================================")
